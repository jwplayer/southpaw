...
public class Southpaw {
  ...
  protected static class Config {
    ...
    public Config(Map<String, Object> rawConfig) throws ClassNotFoundException {
      ...
      this.createRecordsTrigger =
          (int) rawConfig.getOrDefault(CREATE_RECORDS_TRIGGER_CONFIG, CREATE_RECORDS_TRIGGER_DEFAULT);
        ...
        do {
          ...
          if (
            ...
            try (Timer.Context context = metrics.backupsCreated.time()) {
              ...
              backupWatch.reset();
              backupWatch.start();
              if (runWatch.getTime() > runTimeS * 1000 && runTimeS > 0) {
                return;
              }
            }
          ...
          if (childPKs != null) {
            for (ByteArray childPK : childPKs) {
              ...
              if (deChildRecord != null) {
                records.put(childPK, deChildRecord);
              }
            }
          }
  ...
  protected BaseTopic<byte[], DenormalizedRecord> createOutputTopic(String shortName)
    ...
    Class keySerdeClass =
        Class.forName(Preconditions.checkNotNull(topicConfig.get(BaseTopic.KEY_SERDE_CLASS_CONFIG).toString()));
    Class valueSerdeClass =
        Class.forName(Preconditions.checkNotNull(topicConfig.get(BaseTopic.VALUE_SERDE_CLASS_CONFIG).toString()));
    Serde<byte[]> keySerde = (Serde<byte[]>) keySerdeClass.getDeclaredConstructor().newInstance();
    Serde<DenormalizedRecord> valueSerde =
        (Serde<DenormalizedRecord>) valueSerdeClass.getDeclaredConstructor().newInstance();
    return createTopic(
        shortName,
        topicConfig,
        keySerde,
  ...
  protected <K extends BaseRecord, V extends BaseRecord> BaseTopic<K, V> createTopic(String shortName)
    ...
    Class filterClass = Class
        .forName(topicConfig.getOrDefault(BaseTopic.FILTER_CLASS_CONFIG, BaseTopic.FILTER_CLASS_DEFAULT).toString());
  ...
  protected <K, V> BaseTopic<K, V> createTopic(
      ...
      InvocationTargetException {
    Class topicClass =
        Class.forName(Preconditions.checkNotNull(southpawConfig.get(BaseTopic.TOPIC_CLASS_CONFIG).toString()));
  ...
  protected Map<String, Object> createTopicConfig(String configName) {
    ...
    Map<String, Object> defaultConfig =
        new HashMap<>(Preconditions.checkNotNull((Map<String, Object>) topicsConfig.get("default")));
    Map<String, Object> topicConfig =
        new HashMap<>(Preconditions.checkNotNull((Map<String, Object>) topicsConfig.get(configName)));
  ...
  protected AbstractMap.SimpleEntry<Relation, Relation> getRelation(Relation relation, String childEntity) {
    ...
    if (relation.getEntity().equals(childEntity)) {
      return new AbstractMap.SimpleEntry<>(null, relation);
    }
    if (relation.getChildren() == null) {
      return null;
    }
    for (Relation child : relation.getChildren()) {
      if (child.getEntity().equals(childEntity)) {
        return new AbstractMap.SimpleEntry<>(relation, child);
      }
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (Object relation : relations) {
      relURIs.add(new URI(relation.toString()));
    }
  ...
  protected static void validateRootRelations(Relation[] relations) {
    for (Relation relation : relations) {
      ...
      Preconditions.checkNotNull(
          relation.getEntity(),
          String
              .format("Top level relation '%s' must correspond to an input record type", relation.getDenormalizedName())
      );
      Preconditions.checkNotNull(
  ...
  protected void verifyState() {
    for (Map.Entry<String, BaseIndex<BaseRecord, BaseRecord, Set<ByteArray>>> index : fkIndices.entrySet()) {
      ...
      if (missingIndexKeys.isEmpty()) {
        ...
        logger.error("Index " + index.getValue().getIndexedTopic().getShortName() + " check failed for the following " +
            missingIndexKeys.size() + " keys: " + missingIndexKeys.toString());
      }
      ...
      if (missingReverseIndexKeys.isEmpty()) {
        ...
        logger.error(
            "Reverse index " + index.getValue().getIndexedTopic().getShortName() + " check failed for the following " +
                missingReverseIndexKeys.size() + " keys: " + missingReverseIndexKeys.toString());
      }
    }
  }
}
