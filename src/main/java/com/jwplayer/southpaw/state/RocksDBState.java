...
public class RocksDBState extends BaseState {
  ...
  public static class Iterator extends BaseState.Iterator {
    ...
    public AbstractMap.SimpleEntry<byte[], byte[]> next() {
      AbstractMap.SimpleEntry<byte[], byte[]> retVal =
          new AbstractMap.SimpleEntry<>(innerIter.key(), innerIter.value());
      innerIter.next();
      return retVal;
    }
  ...
  public void configure(Map<String, Object> config) {
    try {
      ...
      this.restoreMode =
          RestoreMode.parse(config.getOrDefault(RESTORE_MODE_CONFIG, RestoreMode.NEVER.getValue()).toString());
