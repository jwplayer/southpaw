...
public class ByteArraySetTest {
    public static class LargeEntryTest {
        public void testDeserialize(int numEntries) {
            ByteArraySet oldSet = new ByteArraySet();
            for(int i = 0; i < numEntries; i++) {
                byte[] largeEntry = new byte[200];
                largeEntry[0] = (byte) i;
                oldSet.add(ByteArray.toByteArray(largeEntry));
            }
            ByteArraySet newSet = ByteArraySet.deserialize(oldSet.serialize());
            assertEquals(numEntries, newSet.size());
            assertTrue(newSet.containsAll(oldSet));
        }

        @Test
        public void testMultiChunkDeserialize() {
            testDeserialize(100);
        }

        @Test
        public void testSingleChunkDeserialize() {
            testDeserialize(10);
        }

        @Test(expected = IllegalArgumentException.class)
        public void testAddExcessiveEntrySize() {
            ByteArraySet set = new ByteArraySet();
            byte[] excessiveEntry = new byte[256];
            set.add(ByteArray.toByteArray(excessiveEntry));
        }
    }

    @RunWith(Parameterized.class)
    public static class RandomVariableSizeTest {
